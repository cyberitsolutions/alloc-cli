...
"""
    ...
    def run(self, command_list):
        ...
        # Get vars manually
        if prompt:
            o['task'] = input("Task ID or some text from a task's name, or hit ENTER for none: ")
            if not o['task']:
                o['project'] = input("Project ID or some text from a project's name: ")
            o['hours'] = input("The number of hours you are billing: ")
            o['multiplier'] = input("The rate multiplier (0, 1, 1.5, 2, 3), or hit ENTER for 1: ")
            o['date'] = input("The date that the work was performed, or hit ENTER for %s: " % self.today())
        ...
        if o['interactive']:
            ...
            t.start()
            self.msg(
                'Timer started at ' + str(t.started.replace(microsecond=0)))
            self.msg(
        ...
        # If the time period is expressed like 30m then convert the minutes
        # into hours
        ...
        if float(o['hours']) % .25 != 0:
            self.msg(
                'The hours billed is not a multiple of 15 minutes. Continuing.')
        ...
        # If we've got a taskID, then we know all the details required to add
        # time
        if taskID:
            self.msg(
                "Attempting to add time for work done on this task: %s" % taskID)
            tasks = self.get_list(
                "task", {"taskID": taskID, "taskView": "prioritised"})
            if tasks:
                if not o['quiet']:
                    self.print_table(
        ...
        # ...
            ...
            self.msg(
                "Attempting to add time for work done on this project: %s" % projectID)
            projects = self.get_list("project", {"projectID": projectID})
            if projects:
                self.print_table(
                    "project", projects, ["projectID", "projectName"])
            else:
                self.die("Unable to find project with projectID: %s" %
                         projectID)
        ...
        # We should only have a timeSheetID if a new timeSheetItem was
        # successfully added.
        if timeSheetID:
            if not o['quiet']:
